...
bool Features::command(char *reply, char *command, char *parameter, bool *suppressFrame, bool *numericReply, CommandError *commandError) {
  ...
  if (command[0] == 'F' && command[1] == 'M' && parameter[2] == 0) {
    if (parameter[0] == '?' && parameter[1] == 0) {
      reply[0] = flipMirror.isCamera() ? '1' : '0';
      reply[1] = 0;
      *numericReply = true;
      return true;
    }
    ...
    if (parameter[0] == '0' && parameter[1] == 0) {
      flipMirror.setPosition(false);
      *numericReply = false;
      return true;
    }

    if (parameter[0] == '1' && parameter[1] == 0) {
      flipMirror.setPosition(true);
      *numericReply = false;
      return true;
    }
  ...
  if (command[0] == 'G' && command[1] == 'X' && command[2] == 'M' && command[3] == 0 && parameter[0] == 0) {
    if (transform.mountType == ALTAZM || transform.mountType == ALTAZM_UNL) {
      reply[0] = '0';
      reply[1] = 0;
      *numericReply = true;
      return true;
    ...
    if (transform.mountType == GEM || transform.mountType == GEM_TA || transform.mountType == GEM_TAC ||
        transform.mountType == FORK || transform.mountType == FORK_TA || transform.mountType == FORK_TAC) {
      reply[0] = '1';
      reply[1] = 0;
      *numericReply = true;
      return true;
