...
#ifndef FLIP_MIRROR_ENABLE
  #define FLIP_MIRROR_ENABLE ON
#endif
#ifndef FLIP_MIRROR_SERVO_PIN
  #define FLIP_MIRROR_SERVO_PIN 4
#endif
#ifndef FLIP_MIRROR_CAMERA_DEG
  #define FLIP_MIRROR_CAMERA_DEG 30
#endif
#ifndef FLIP_MIRROR_EYEPIECE_DEG
  #define FLIP_MIRROR_EYEPIECE_DEG 90
#endif
...
class FlipMirrorController {
public:
  void init(int pin) {
    servoPin = pin;
    if (FLIP_MIRROR_ENABLE != ON || servoPin == OFF) {
      initialized = false;
  ...
  void setPosition(bool camera) {
    currentPosition = camera;
    if (initialized) {
      const int angle = camera ? FLIP_MIRROR_CAMERA_DEG : FLIP_MIRROR_EYEPIECE_DEG;
      servo.write(angle);
    }
  }

  bool isCamera() const { return currentPosition; }
...
private:
    ...
    Servo servo;
  #endif
  int servoPin = FLIP_MIRROR_SERVO_PIN;
...
public:
  ...
  private:
    ...
    uint16_t nvKey[8];
    FlipMirrorController flipMirror;
};
